...
def TCCNet(input_shape, classes=53, nb_stacks=4, nb_filters=8,
    filters=[32, 32, 64, 128], filters_size=3,
    n_dropout=0.05, n_l2=0.0005, classify_as='aot', masking=False):
    """
        Arguments:
    ...
    model_tcn = TCN(input_shape=input_shape, nb_stacks=4, nb_filters=8,
                dilations=filters, kernel_size=3,
                dropout_rate=n_dropout)
    # , weight_decay=n_l2, seed=0, masking=masking
    ...
    x = Bidirectional(LSTM(32, return_sequences=False))(x_input)
...
def getNetwork():
    ...
    model = TCCNet(input_shape=(None, 10), classes=53)
